...
function updateMapMarkers() {
    ...
    state.itinerary.forEach(activity => {
        if (activity.location && activity.location.trim() !== '') {
            ...
            const infoWindow = new google.maps.InfoWindow({
                content: `<b>${activity.name}</b>${activity.time ? `<br>${activity.time}` : ''}`
            });
    ...
    if (locations.length > 0) {
        locations.forEach(location => {
            ...
            li.innerHTML = `<i class="fas fa-map-marker-alt"></i> ${location.name}${location.time ? ` (${location.time})` : ''}`;
    ...
    if (state.infoItems.car.length > 0) {
        ...
        // 更新文本顯示
        document.getElementById('pickup-location').textContent = carInfo.pickUpLocation || '未設定';
        document.getElementById('return-location').textContent = carInfo.returnLocation || '未設定';
        
        // 添加租車地點標記
        addRentalMarkers(carInfo);
    } else {
        document.getElementById('pickup-location').textContent = '未設定';
        document.getElementById('return-location').textContent = '未設定';
    }
}

// 添加租車地點標記
function addRentalMarkers(carInfo) {
    if (!state.mapInitialized) return;
    
    console.log('添加租車地點標記:', carInfo);
    
    // 取車地點標記 (綠色)
    if (carInfo.pickUpLocation && carInfo.pickUpLocation.trim() !== '' && carInfo.pickUpLocation !== '未設定') {
        const pickupLat = 13.6811 + (Math.random() - 0.5) * 0.01;
        const pickupLng = 100.7471 + (Math.random() - 0.5) * 0.01;
        ...
        const pickupInfoWindow = new google.maps.InfoWindow({
            content: `<b>取車地點</b><br>${carInfo.pickUpLocation}${carInfo.pickUpTime ? `<br>時間: ${carInfo.pickUpTime}` : ''}`
        });
        ...
        console.log('取車地點標記已添加:', carInfo.pickUpLocation);
    }
    
    // 還車地點標記 (紅色) - 修復：確保還車地點也被添加
    if (carInfo.returnLocation && carInfo.returnLocation.trim() !== '' && carInfo.returnLocation !== '未設定') {
        const returnLat = 13.7462 + (Math.random() - 0.5) * 0.01;
        const returnLng = 100.5347 + (Math.random() - 0.5) * 0.01;
        
        const returnMarker = new google.maps.Marker({
            position: { lat: returnLat, lng: returnLng },
            map: state.map,
        ...
        const returnInfoWindow = new google.maps.InfoWindow({
            content: `<b>還車地點</b><br>${carInfo.returnLocation}${carInfo.returnTime ? `<br>時間: ${carInfo.returnTime}` : ''}`
        });
        ...
        console.log('還車地點標記已添加:', carInfo.returnLocation);
    }
}
    ...
    switch(type) {
        ...
        case 'car':
            fields = [
                ...
                {id: 'pick-up-location', label: '取車地點', type: 'text', required: true},
                {id: 'return-location', label: '還車地點', type: 'text', required: true}
            ];
...
function addActivity(e) {
    ...
    const activity = {
        ...
        time: document.getElementById('activity-time').value || null, // 時間可選
...
function addInfoItem(e) {
    ...
    switch(type) {
        ...
        case 'car':
            ...
            item.pickUpLocation = document.getElementById('pick-up-location').value.trim() || '未設定';
            item.returnLocation = document.getElementById('return-location').value.trim() || '未設定';
            console.log('租車資訊保存:', item);
            break;
...
function renderItinerary() {
    ...
    dates.forEach((date, index) => {
        ...
        activities.forEach(activity => {
            ...
            // 時間顯示（如果有的話）
            const timeDisplay = activity.time ? 
                `<div class="activity-time">${activity.time}</div>` : 
                `<div class="activity-time" style="background: rgba(160, 174, 192, 0.1); color: #a0aec0;">未設定</div>`;
...
function renderBudgetItems() {
    ...
    console.log('支出渲染完成，共', state.budgetItems.length, '個項目');
}
...
function updateTotalExpenses() {
    ...
    if (totalExpensesElement) {
        totalExpensesElement.textContent = `฿${totalSpent.toLocaleString()}`;
    }
    
    console.log('總支出更新:', totalSpent);
}
        ...
        switch(type) {
            ...
            case 'car':
                ...
                content = `
                    <div class="info-field">
                        <strong>取車時間:</strong>
                        <span>${item.pickUpTime || '未設定'}</span>
                    </div>
                    <div class="info-field">
                        <strong>還車時間:</strong>
                        <span>${item.returnTime || '未設定'}</span>
                    </div>
                    <div class="info-field">
                        <strong>取車地點:</strong>
                        <span>${item.pickUpLocation || '未設定'}</span>
                    </div>
                    <div class="info-field">
                        <strong>還車地點:</strong>
                        <span>${item.returnLocation || '未設定'}</span>
                    </div>
                `;
                break;
